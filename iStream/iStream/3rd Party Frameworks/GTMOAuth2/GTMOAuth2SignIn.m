...
- (BOOL)requestRedirectedToRequest:(NSURLRequest *)redirectedRequest {
    ...
    if (!self.hasHandledCallback) {
        ...
        // Thomas 1.5.2012: Foursquare special fallback
        // The Foursquare access_token is not in the query part of the response URL but looks sth like this:
        // http://www.tttthomasssss.com/#access_token=FDAEWQVVO3H3SQYANRXVPFZXTRZY2WY2JV04CBFQJGZID103
        // So, we need to check for the hashy-hash ('#') in case of 4sq...
        if (!responseStr) {
            if ([[[self authentication] serviceProvider] isEqualToString:MTServiceTypeFoursquare]) {// TODO: replace the hardcoded token
                responseStr = [[[[redirectedRequest URL] absoluteString] componentsSeparatedByString:@"#"] lastObject];
            }
        } else {
#if DEBUG
...
- (void)handleCallbackReached {
    ...
    // Thomas: 1.5.2012: Its the accessToken stupid!
    // In case of 4sq, we don't need to request a code,
    // and in exchange of a code get the accesToken, NO,
    // we get the accessToken straight away, so we check here
    // if we already have a nice little accessToken before 
    // trying to go through the rest of the OAuth clusterfuck
    ...
    if ([accessToken length] > 0) {
        ...
        [self auth:auth finishedWithFetcher:nil error:nil];
